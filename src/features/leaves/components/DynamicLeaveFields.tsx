...
export function DynamicLeaveFields({ schema, register }: { schema: LeaveFormSchema; register: UseFormRegister<CreateLeaveDto> }) {
  ...
  return <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
    ...
    <div className="space-y-4">{schema.fields.map((field) => {
      ...
      const common = { ...registration, required: field.required, placeholder: field.placeholder, minLength: field.minLength, maxLength: field.maxLength, className: "w-full rounded-lg border border-input bg-background px-3 py-2 text-body" };
      return <label key={field.key} className="block text-body"><span className="mb-1 block font-medium">{field.label}{field.required ? <span className="text-destructive"> *</span> : ""}</span>
