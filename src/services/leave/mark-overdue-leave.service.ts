...
export async function markOverdueSingleLeave(
  ...
  return await transaction(async (tx) => {
    ...
    // The return QR is intentionally NOT invalidated: an overdue student may
    // still use it to check back in and complete the leave.
...
export async function markOverdueLeaves(
  ...
  const now = new Date();
  const overdueLeaves = await leaveRepository.findOverdueLeaves(now);

  const result: MarkOverdueBatchResult = {
    total: overdueLeaves.length,
    overdue: 0,
    skipped: 0,
    errors: [],
  };
