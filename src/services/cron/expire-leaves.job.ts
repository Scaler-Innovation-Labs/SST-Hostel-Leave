...
import { expireAuthorizations } from "@/services/leave/recurring-authorization/expire-authorizations.service";
...
export type ExpireLeavesJobResult = {
	...
	/** Recurring authorizations past validUntil → EXPIRED. */
	authorizationsExpired: { count: number };
};
...
export async function runExpireLeavesJob(): Promise<ExpireLeavesJobResult> {
	...
	const authorizationsExpired = await expireAuthorizations(SYSTEM_USER);
